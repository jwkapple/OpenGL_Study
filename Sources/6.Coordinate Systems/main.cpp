...
int main()
{
 ...
 glm::vec3 cubePositions[] = {
  ...
  glm::vec3(-1.3f,  1.0f, -1.5f)
	...
	projection = glm::perspective(glm::radians(45.0f), (float)SCREEN_WIDTH / SCREEN_HEIGHT, 1.0f, 100.0f);

	int modelLoca = glGetUniformLocation(ourShader.ID, "model");
	glUniformMatrix4fv(modelLoca, 1, GL_FALSE, glm::value_ptr(model));

	int viewLoca = glGetUniformLocation(ourShader.ID, "view");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		rotation = glm::rotate(rotation,  glm::radians(50.0f) , glm::vec3(0.5f, 1.0f, 0.0f));
		glUniformMatrix4fv(glGetUniformLocation(ourShader.ID, "rotation"), 1, GL_FALSE, glm::value_ptr(rotation));
